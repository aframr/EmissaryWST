...
describe('Employee', function () {
  ...
  var credentials = void 0; // variable to hold all the need authentication variables.
  ...
  describe('Employee Testing', function () {
    ...
    describe('POST /api/employees', function () {
      it('should save submitted employee', function (done) {
        request(url).post('/api/employees').query({ email: credentials.email, token: credentials.token }).send({
    ...
    describe('Login', function () {
      it('Should login with employee data', function (done) {
        request(url).post('/api/employees/login').send({
          email: 'jt@tomcruise.com',
          password: 'test'
        }).end(function (err, res) {
          if (err) throw err;
      ...
      it('Should update the employee data', function (done) {
        request(url).put('/api/employees/' + returnedId).query({ email: credentials.email, token: credentials.token }).send({
    ...
    describe('GET /api/employees/company/:id', function () {
      it('should return all employees', function (done) {
        request(url).get('/api/employees/company/' + credentials.admin._id).send({
          _admin_id: credentials.admin._id
        }).end(function (err, res) {
    ...
    describe('GET /api/employees/:id', function () {
      it('should return a specific employee', function (done) {
        request(url).get('/api/employees/' + returnedId).query({ email: credentials.email, token: credentials.token }).end(function (err, res) {
          res.body.should.have.property('_id');
          res.body.should.have.property('email');
    ...
    describe('DELETE /api/employees/:id', function () {
      it('Should delete the employee data', function (done) {
        request(url).delete('/api/employees/' + returnedId).query({ email: credentials.email, token: credentials.token }).end(function (err, res) {
          res.body.should.have.property('_id');
          res.body.should.not.have.property('password');
          done();
        });
