...
describe('VisitorList', function () {
  ...
  it('should add vistitors to list', function (done) {
    request(url).post('/api/visitorLists').send({
  ...
  it('should get visitor list', function (done) {
    this.timeout(8000);
    request(url).get('/api/visitorLists/company/' + currCompany._id).send().expect(200).end(function (err, res) {
  ...
  it('should not get visitor list', function (done) {
    this.timeout(8000);
    request(url).get('/api/visitorLists/company/0').send().expect(404).end(function (err, res) {
  ...
  it('should delete specified Visitor', function (done) {
    request(url).get('/api/visitorLists/company/' + currCompany._id).end(function (err, res) {
      var prevLen = 0;
      ...
      request(url).delete('/api/visitorLists/company/' + currCompany._id + '/visitor/' + visitor1._id).expect(200).end(function (err, res) {
  ...
  it('should clear visitorLists', function (done) {
    this.timeout(8000);
    request(url).delete('/api/visitorLists/' + currVisitorList._id).expect(200).end(function (err, res) {
      res.body.should.have.property('visitors');
