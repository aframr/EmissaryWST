...
describe('Company Test', function () {
  ...
  before(function (done) {
    request(url).post('/api/companies').send({
      email: email,
      name: name,
  ...
  it('should get company', function (done) {
    request(url).get('/api/companies/' + currCompany._id).expect(200).end(function (err, res) {
      res.body.should.have.property('_id');
      done();
    });
  });
  ...
  it('should not get company', function (done) {
    request(url).get('/api/companies/' + 0).expect(400).end(function (err, res) {
  ...
  it('should get all companies', function (done) {
    request(url).get('/api/companies').expect(200).end(function (err, res) {
  ...
  it('should update company', function (done) {
    request(url).put('/api/companies/' + currCompany._id).send({
      email: new_email,
      name: new_name,
  ...
  it('should delete company', function (done) {
    request(url).delete('/api/companies/' + currCompany._id).expect(200).end(function (err, res) {
      res.body.should.have.property('_id');
