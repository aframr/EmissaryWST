...
describe('Appointment Test', function () {
  ...
  before(function (done) {
    ...
    company.save(function (err, c) {
      currCompany = c;
      request(url).post('/api/appointments').send({
  ...
  it('should get appointment', function (done) {
    request(url).get('/api/appointments/' + currAppointment._id).expect(200).end(function (err, res) {
      res.body.should.have.property('_id');
      done();
    });
  });
  ...
  it('should not get appointment', function (done) {
    request(url).get('/api/appointments/' + 0).expect(400).end(function (err, res) {
  ...
  it('should get all appointments', function (done) {
    request(url).get('/api/appointments/company/' + currCompany._id).expect(200).end(function (err, res) {
  ...
  it('should update appointment', function (done) {
    request(url).put('/api/appointments/' + currAppointment._id).send({
      first_name: new_first_name,
      last_name: new_last_name,
      phone_number: new_phone_number,
      date: new_date,
  ...
  it('should delete appointment', function (done) {
    request(url).delete('/api/appointments/' + currAppointment._id).expect(200).end(function (err, res) {
      res.body.should.have.property('_id');
