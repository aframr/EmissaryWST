...
/** ******** Socket IO Module **********/
_exports.createServer = function (io_in) {
  io = io_in;
    ...
    socket.on(VALIDATE_COMPANY_ID, function (data) {
      ...
      Company.findOne({ _id: company_id }, function (err, c) {
        ...
        VisitorListCtr.getCompanyVisitorList(company_id, function (err_msg, result) {
          if (err_msg) {
            _exports.notifyError(company_id, { error: err_msg });
          } else {
            _exports.notifyNewList(company_id, result);
          }
        });
      });
    });
 ...
 */
_exports.notifyNewList = function (company_id, data) {
  io.to(company_id).emit(VISITOR_LIST_UPDATE, data);
};

_exports.notifyError = function (company_id, data) {
